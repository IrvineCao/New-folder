    ...
    "count": """
        WITH
            dim_data AS (
                ...
                WHERE
                    sf.ads_ops_storefront_id IN (4217, 3500)
                    AND ws.id = '53003'
                    AND ws_tag.name IN ('FABEN', 'FABCLEAN', 'HHC')
            ),
            sos_metrics AS (
                SELECT
                    s.timing,
                ...
                WHERE true
                    ...
                    AND s.timing = 'daily'
                    AND DATE(created_datetime) in ('2025-1-1', '2025-1-15', '2025-2-2')
                GROUP BY
                    s.keyword_id,
                    s.storefront_id,
        ...
        FROM (SELECT 1
        FROM main_data
        GROUP BY workspace_id,storefront_id,keyword_ws_id,Month)
    """,
    "data": """
        WITH
            dim_data AS (
